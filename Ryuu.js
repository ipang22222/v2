...
reply(`Itu Kak Nomor Owner Ku Jangan Lupa Save Yahh`)
break
case 'mode':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.owner) 
...
if (isLimit(sender, isPrem, isOwner, limitCount, limit)) return reply(mess.limit) 
if (isBanned) return reply('Kamu Sudah Di banned!')
if (args.length == 0) return reply(`Example: ${prefix}${command} Atsair Botz`)
reply(mess.wait) 
anu = args.join(" ")
...
if (isBanned) return reply('Kamu Sudah Di banned!')
if (args.length == 0) return reply(`Example: ${prefix}${command} AtsairBotz`)
reply(mess.wait) 
anu = args.join(" ") 
